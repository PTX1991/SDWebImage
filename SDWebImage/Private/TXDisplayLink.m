...
#define kTXDisplayLinkInterval 1.0 / 60

@interface TXDisplayLink ()
...
#elif SD_IOS || SD_TV
        TXWeakProxy *weakProxy = [TXWeakProxy proxyWithTarget:self];
        _displayLink = [CADisplayLink displayLinkWithTarget:weakProxy selector:@selector(displayLinkDidRefresh:)];
#else
        TXWeakProxy *weakProxy = [TXWeakProxy proxyWithTarget:self];
        _displayLink = [NSTimer timerWithTimeInterval:kTXDisplayLinkInterval target:weakProxy selector:@selector(displayLinkDidRefresh:) userInfo:nil repeats:YES];
#endif
    }
    return self;
}
...
+ (instancetype)displayLinkWithTarget:(id)target selector:(SEL)sel {
    TXDisplayLink *displayLink = [[TXDisplayLink alloc] initWithTarget:target selector:sel];
    return displayLink;
}
...
#endif
    if (duration <= 0) {
        duration = kTXDisplayLinkInterval;
    }
    return duration;
}
...
- (void)start {
    ...
    if (self.displayLink.isValid) {
        ...
        TXWeakProxy *weakProxy = [TXWeakProxy proxyWithTarget:self];
        self.displayLink = [NSTimer timerWithTimeInterval:kTXDisplayLinkInterval target:weakProxy selector:@selector(displayLinkDidRefresh:) userInfo:nil repeats:YES];
...
static CVReturn DisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow, const CVTimeStamp *inOutputTime, CVOptionFlags flagsIn, CVOptionFlags *flagsOut, void *displayLinkContext) {
    ...
    TXDisplayLink *object = (__bridge TXDisplayLink *)displayLinkContext;
    if (inOutputTime) {
        object.outputTime = *inOutputTime;
    }
    __weak TXDisplayLink *weakObject = object;
