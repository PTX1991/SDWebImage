 ...
 */
@interface TXImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `TXImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TXImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `TXImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) TXImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `TXMemoryCache` protocol to allow usage.
 * Defaults to built-in `TXMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `TXDiskCache` protocol to allow usage.
 * Defaults to built-in `TXDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
