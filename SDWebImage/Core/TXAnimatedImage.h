...
/**
 This is the protocol for TXAnimatedImage class only but not for TXAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol TXAnimatedImage <TXAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable TXImageCoderOptions *)options;
...
/**
 ...
 @param animatedCoder An animated coder which conform `TXAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<TXAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 ...
 @note We use this with animated coder which conforms to `SDProgressiveImageCoder` for progressive animation decoding.
 */
@property (nonatomic, strong, readonly, nullable) id<TXAnimatedImageCoder> animatedCoder;

@end

/**
 The image class which supports animating on `TXAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
@interface TXAnimatedImage : UIImage <TXAnimatedImage>
...
// Pay attention, when the animated image frame count <= 1, all the `TXAnimatedImageProvider` protocol methods will return nil or 0 value, you'd better check the frame count before usage and keep fallback.
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the description in `TXAnimatedImage` protocol.
