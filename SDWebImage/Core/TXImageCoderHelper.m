...
@implementation TXImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<TXImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(TXImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    TXAnimatedImageRep *imageRep = [[TXAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<TXImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            TXImageFrame *frame = [TXImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            TXImageFrame *frame = [TXImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    // on iOS7, do not forget to call [[TXImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
