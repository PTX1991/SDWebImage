...
typedef NSData * _Nullable (^TXWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol TXWebImageDownloaderDecryptor <NSObject>
...
/// @note If nil is returned, the image download will be marked as failed with error `TXWebImageErrorBadImageData`
...
*/
@interface TXWebImageDownloaderDecryptor : NSObject <TXWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull TXWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull TXWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface TXWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) TXWebImageDownloaderDecryptor *base64Decryptor;
