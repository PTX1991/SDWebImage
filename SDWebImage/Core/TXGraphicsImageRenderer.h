...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `TXImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

typedef void (^SDGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, TXGraphicsImageRendererFormatRange) {
    TXGraphicsImageRendererFormatRangeUnspecified = -1,
    TXGraphicsImageRendererFormatRangeAutomatic = 0,
    TXGraphicsImageRendererFormatRangeExtended,
    TXGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface TXGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) TXGraphicsImageRendererFormatRange preferredRange;
...
@interface TXGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A TXGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull TXGraphicsImageRendererFormat *)format;
