...
#import "TXWebImagePrefetcher.h"
#import "TXAsyncBlockOperation.h"
...
@property (nonatomic, weak) TXWebImagePrefetcher *prefetcher;
@property (nonatomic, copy, nullable) TXWebImagePrefetcherCompletionBlock completionBlock;
@property (nonatomic, copy, nullable) TXWebImagePrefetcherProgressBlock progressBlock;
...
@implementation TXWebImagePrefetcher
...
- (nonnull instancetype)init {
    return [self initWithImageManager:[TXWebImageManager new]];
}

- (nonnull instancetype)initWithImageManager:(TXWebImageManager *)manager {
    if ((self = [super init])) {
        _manager = manager;
...
- (void)startPrefetchWithToken:(SDWebImagePrefetchToken * _Nonnull)token {
    for (NSURL *url in token.urls) {
        @autoreleasepool {
            @weakify(self);
            TXAsyncBlockOperation *prefetchOperation = [TXAsyncBlockOperation blockOperationWithBlock:^(TXAsyncBlockOperation * _Nonnull asyncOperation) {
                @strongify(self);
                if (!self || asyncOperation.isCancelled) {
                    return;
                }
                id<TXWebImageOperation> operation = [self.manager loadImageWithURL:url options:self.options context:self.context progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, TXImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                    @strongify(self);
                    if (!self) {
                        return;
                    }
                ...
                NSAssert(operation != nil, @"Operation should not be nil, [TXWebImageManager loadImageWithURL:options:context:progress:completed:] break prefetch logic");
...
- (void)cancel {
    ...
    for (id operation in self.prefetchOperations) {
        id<TXWebImageOperation> strongOperation = operation;
        if (strongOperation) {
