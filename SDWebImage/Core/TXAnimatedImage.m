...
@interface TXAnimatedImage ()

@property (nonatomic, strong) id<TXAnimatedImageCoder> animatedCoder;
@property (nonatomic, assign, readwrite) SDImageFormat animatedImageFormat;
@property (atomic, copy) NSArray<TXImageFrame *> *loadedAnimatedImageFrames; // Mark as atomic to keep thread-safe
@property (nonatomic, assign, getter=isAllFramesLoaded) BOOL allFramesLoaded;

@end

@implementation TXAnimatedImage
...
+ (instancetype)imageNamed:(NSString *)name inBundle:(NSBundle *)bundle scale:(CGFloat)scale {
    ...
    TXImageAssetManager *assetManager = [TXImageAssetManager sharedAssetManager];
    TXAnimatedImage *image = (TXAnimatedImage *)[assetManager imageForName:name];
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale options:(TXImageCoderOptions *)options {
    if (!data || data.length == 0) {
        return nil;
    }
    data = [data copy]; // avoid mutable data
    id<TXAnimatedImageCoder> animatedCoder = nil;
    for (id<TXImageCoder>coder in [TXImageCodersManager sharedManager].coders.reverseObjectEnumerator) {
        if ([coder conformsToProtocol:@protocol(TXAnimatedImageCoder)]) {
            if ([coder canDecodeFromData:data]) {
                if (!options) {
                    options = @{TXImageCoderDecodeScaleFactor : @(scale)};
                }
...
- (instancetype)initWithAnimatedCoder:(id<TXAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale {
    if (!animatedCoder) {
        return nil;
    }
...
- (void)preloadAllFrames {
    ...
    if (!self.isAllFramesLoaded) {
        NSMutableArray<TXImageFrame *> *frames = [NSMutableArray arrayWithCapacity:self.animatedImageFrameCount];
        for (size_t i = 0; i < self.animatedImageFrameCount; i++) {
            ...
            TXImageFrame *frame = [TXImageFrame frameWithImage:image duration:duration]; // through the image should be nonnull, used as nullable for `animatedImageFrameAtIndex:`
        ...
        for (id<TXImageCoder>coder in [TXImageCodersManager sharedManager].coders.reverseObjectEnumerator) {
            if ([coder conformsToProtocol:@protocol(TXAnimatedImageCoder)]) {
                if ([coder canDecodeFromData:animatedImageData]) {
                    animatedCoder = [[[coder class] alloc] initWithAnimatedImageData:animatedImageData options:@{TXImageCoderDecodeScaleFactor : @(scale)}];
                    break;
                }
            }
        }
...
#pragma mark - TXAnimatedImageProvider
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    ...
    if (self.isAllFramesLoaded) {
        TXImageFrame *frame = [self.loadedAnimatedImageFrames objectAtIndex:index];
        return frame.image;
    }
...
@implementation TXAnimatedImage (MemoryCacheCost)
...
@implementation TXAnimatedImage (Metadata)
...
@implementation TXAnimatedImage (MultiFormat)
...
+ (nullable UIImage *)sd_imageWithData:(nullable NSData *)data scale:(CGFloat)scale firstFrameOnly:(BOOL)firstFrameOnly {
    ...
    return [[self alloc] initWithData:data scale:scale options:@{TXImageCoderDecodeFirstFrameOnly : @(firstFrameOnly)}];
}
...
- (nullable NSData *)sd_imageDataAsFormat:(SDImageFormat)imageFormat compressionQuality:(double)compressionQuality firstFrameOnly:(BOOL)firstFrameOnly {
    ...
    NSMutableArray<TXImageFrame *> *frames = [NSMutableArray arrayWithCapacity:frameCount];
    ...
    UIImage *animatedImage = [TXImageCoderHelper animatedImageWithFrames:frames];
