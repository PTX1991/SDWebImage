...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conforms to `TXAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface TXAnimatedImageView : UIImageView
/**
 ...
 @warning Pay attention if you directly update the player's property like `totalFrameCount`, `totalLoopCount`, the same property on `TXAnimatedImageView` may not get synced.
 */
@property (nonatomic, strong, readonly, nullable) TXAnimatedImagePlayer *player;
...
/// Asynchronous setup animation playback mode. Default mode is TXAnimatedImagePlaybackModeNormal.
@property (nonatomic, assign) TXAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `TXAnimatedImage` with animated coder which conform to `SDProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `TXAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
