...
@interface TXWebImageDownloaderOperation ()

@property (strong, nonatomic, nonnull) NSMutableArray<SDCallbacksDictionary *> *callbackBlocks;

@property (assign, nonatomic, readwrite) TXWebImageDownloaderOptions options;
@property (copy, nonatomic, readwrite, nullable) SDWebImageContext *context;

@property (assign, nonatomic, getter = isExecuting) BOOL executing;
@property (assign, nonatomic, getter = isFinished) BOOL finished;
@property (strong, nonatomic, nullable) NSMutableData *imageData;
@property (copy, nonatomic, nullable) NSData *cachedData; // for `TXWebImageDownloaderIgnoreCachedResponse`
...
@property (strong, nonatomic, nullable) id<TXWebImageDownloaderResponseModifier> responseModifier; // modify original URLResponse
@property (strong, nonatomic, nullable) id<TXWebImageDownloaderDecryptor> decryptor; // decrypt image data
...
@implementation TXWebImageDownloaderOperation
...
- (BOOL)cancel:(nullable id)token {
    ...
    if (shouldCancel) {
        ...
        TXWebImageDownloaderCompletedBlock completedBlock = [token valueForKey:kCompletedCallbackKey];
        dispatch_main_async_safe(^{
            if (completedBlock) {
                completedBlock(nil, nil, [NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorCancelled userInfo:@{NSLocalizedDescriptionKey : @"Operation cancelled by user during sending the request"}], YES);
            }
        });
    }
    return shouldCancel;
...
- (void)start {
    @synchronized (self) {
        if (self.isCancelled) {
            ...
            [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorCancelled userInfo:@{NSLocalizedDescriptionKey : @"Operation cancelled by user before sending the request"}]];
            [self reset];
            return;
        }
        ...
        if (self.options & TXWebImageDownloaderIgnoreCachedResponse) {
        ...
        if (!session.delegate) {
            ...
            [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorInvalidDownloadOperation userInfo:@{NSLocalizedDescriptionKey : @"Session delegate is nil and invalid"}]];
            [self reset];
            return;
        }
    ...
    if (self.dataTask) {
        if (self.options & TXWebImageDownloaderHighPriority) {
            self.dataTask.priority = NSURLSessionTaskPriorityHigh;
            self.coderQueue.qualityOfService = NSQualityOfServiceUserInteractive;
        } else if (self.options & TXWebImageDownloaderLowPriority) {
        ...
        for (TXWebImageDownloaderProgressBlock progressBlock in [self callbacksForKey:kProgressCallbackKey]) {
        ...
        if (!self.isFinished) self.finished = YES;
        [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorInvalidDownloadOperation userInfo:@{NSLocalizedDescriptionKey : @"Task can't be initialized"}]];
        [self reset];
    }
}
...
- (void)cancelInternal {
    ...
    [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorCancelled userInfo:@{NSLocalizedDescriptionKey : @"Operation cancelled by user during sending the request"}]];
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    if (self.responseModifier && response) {
        ...
        if (!response) {
            valid = NO;
            self.responseError = [NSError errorWithDomain:TXWebImageErrorDomain
                                                     code:TXWebImageErrorInvalidDownloadResponse
    ...
    if (!statusCodeValid) {
        valid = NO;
        self.responseError = [NSError errorWithDomain:TXWebImageErrorDomain
                                                 code:TXWebImageErrorInvalidDownloadStatusCode
                                             userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithFormat:@"Download marked as failed because of invalid response status code %ld", (long)statusCode],
                                                        TXWebImageErrorDownloadStatusCodeKey : @(statusCode),
                                                        TXWebImageErrorDownloadResponseKey : response}];
    }
    ...
    if (!contentTypeValid) {
        valid = NO;
        self.responseError = [NSError errorWithDomain:TXWebImageErrorDomain
                                                 code:TXWebImageErrorInvalidDownloadContentType
                                             userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithFormat:@"Download marked as failed because of invalid response content type %@", contentType],
                                                        TXWebImageErrorDownloadContentTypeKey : contentType,
                                                        TXWebImageErrorDownloadResponseKey : response}];
    }
    ...
    if (valid && statusCode == 304 && !self.cachedData) {
        valid = NO;
        self.responseError = [NSError errorWithDomain:TXWebImageErrorDomain
                                                 code:TXWebImageErrorCacheNotModified
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    BOOL supportProgressive = (self.options & TXWebImageDownloaderProgressiveLoad) && !self.decryptor;
    ...
    if (supportProgressive && !finished) {
        ...
        if (self.coderQueue.operationCount == 0) {
            ...
            [self.coderQueue addOperationWithBlock:^{
                ...
                UIImage *image = TXImageLoaderDecodeProgressiveImageData(imageData, self.request.URL, NO, self, [[self class] imageOptionsFromDownloaderOptions:self.options], self.context);
                if (image) {
...
- (void)URLSession:(NSURLSession *)session
 ...
 completionHandler:(void (^)(NSCachedURLResponse *cachedResponse))completionHandler {
    ...
    if (!(self.options & TXWebImageDownloaderUseNSURLCache)) {
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error) {
        ...
        if ([self callbacksForKey:kCompletedCallbackKey].count > 0) {
            ...
            if (imageData) {
                /**  if you specified to only use cached data via `TXWebImageDownloaderIgnoreCachedResponse`,
                 *  then we should check if the cached data is equal to image data
                 */
                if (self.options & TXWebImageDownloaderIgnoreCachedResponse && [self.cachedData isEqualToData:imageData]) {
                    self.responseError = [NSError errorWithDomain:TXWebImageErrorDomain
                                                             code:TXWebImageErrorCacheNotModified
                                                         userInfo:@{NSLocalizedDescriptionKey : @"Downloaded image is not modified and ignored",
                                                                    TXWebImageErrorDownloadResponseKey : self.response}];
                    ...
                    [self.coderQueue addOperationWithBlock:^{
                        ...
                        id<SDProgressiveImageCoder> progressiveCoder = TXImageLoaderGetProgressiveCoder(self);
                        UIImage *image;
                        if (progressiveCoder) {
                            image = TXImageLoaderDecodeProgressiveImageData(imageData, self.request.URL, YES, self, [[self class] imageOptionsFromDownloaderOptions:self.options], self.context);
                        } else {
                            image = TXImageLoaderDecodeImageData(imageData, self.request.URL, [[self class] imageOptionsFromDownloaderOptions:self.options], self.context);
                        }
                        CGSize imageSize = image.size;
                        if (imageSize.width == 0 || imageSize.height == 0) {
                            NSString *description = image == nil ? @"Downloaded image decode failed" : @"Downloaded image has 0 pixels";
                            [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorBadImageData userInfo:@{NSLocalizedDescriptionKey : description}]];
                        } else {
                ...
                [self callCompletionBlocksWithError:[NSError errorWithDomain:TXWebImageErrorDomain code:TXWebImageErrorBadImageData userInfo:@{NSLocalizedDescriptionKey : @"Image data is nil"}]];
                [self done];
            }
        } else {
            [self done];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential *credential))completionHandler {
    ...
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        if (!(self.options & TXWebImageDownloaderAllowInvalidSSLCertificates)) {
...
+ (SDWebImageOptions)imageOptionsFromDownloaderOptions:(TXWebImageDownloaderOptions)downloadOptions {
    SDWebImageOptions options = 0;
    if (downloadOptions & TXWebImageDownloaderScaleDownLargeImages) options |= SDWebImageScaleDownLargeImages;
    if (downloadOptions & TXWebImageDownloaderDecodeFirstFrameOnly) options |= SDWebImageDecodeFirstFrameOnly;
    if (downloadOptions & TXWebImageDownloaderPreloadAllFrames) options |= SDWebImagePreloadAllFrames;
    if (downloadOptions & TXWebImageDownloaderAvoidDecodeImage) options |= SDWebImageAvoidDecodeImage;
    if (downloadOptions & TXWebImageDownloaderMatchAnimatedImageClass) options |= SDWebImageMatchAnimatedImageClass;
    
    return options;
}

- (BOOL)shouldContinueWhenAppEntersBackground {
    return SD_OPTIONS_CONTAINS(self.options, TXWebImageDownloaderContinueInBackground);
}
...
- (void)callCompletionBlocksWithImage:(nullable UIImage *)image
    ...
    dispatch_main_async_safe(^{
        for (TXWebImageDownloaderCompletedBlock completedBlock in completionBlocks) {
