...
typedef void(^SDExternalCompletionBlock)(UIImage * _Nullable image, NSError * _Nullable error, TXImageCacheType cacheType, NSURL * _Nullable imageURL);

typedef void(^SDInternalCompletionBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, TXImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL);

/**
 A combined operation representing the cache and loader operation. You can use it to cancel the load process.
 */
@interface SDWebImageCombinedOperation : NSObject <TXWebImageOperation>
 ...
 */
@property (strong, nonatomic, nullable, readonly) id<TXWebImageOperation> cacheOperation;

/**
 The loader operation from the image loader (such as download operation)
 */
@property (strong, nonatomic, nullable, readonly) id<TXWebImageOperation> loaderOperation;

@end


@class TXWebImageManager;

/**
 The manager delegate protocol.
 */
@protocol TXWebImageManagerDelegate <NSObject>
 ...
 *
 * @param imageManager The current `TXWebImageManager`
 * @param imageURL     The url of the image to be downloaded
 *
 * @return Return NO to prevent the downloading of the image on cache misses. If not implemented, YES is implied.
 */
- (BOOL)imageManager:(nonnull TXWebImageManager *)imageManager shouldDownloadImageForURL:(nonnull NSURL *)imageURL;
 ...
 */
- (BOOL)imageManager:(nonnull TXWebImageManager *)imageManager shouldBlockFailedURL:(nonnull NSURL *)imageURL withError:(nonnull NSError *)error;

@end

/**
 * The TXWebImageManager is the class behind the UIImageView+WebCache category and likes.
 * It ties the asynchronous downloader (TXWebImageDownloader) with the image cache store (TXImageCache).
 ...
 * @endcode
 */
@interface TXWebImageManager : NSObject

/**
 * The delegate for manager. Defaults to nil.
 */
@property (weak, nonatomic, nullable) id <TXWebImageManagerDelegate> delegate;

/**
 * The image cache used by manager to query image cache.
 */
@property (strong, nonatomic, readonly, nonnull) id<TXImageCache> imageCache;

/**
 * The image loader used by manager to load image.
 */
@property (strong, nonatomic, readonly, nonnull) id<TXImageLoader> imageLoader;

/**
 The image transformer for manager. It's used for image transform after the image load finished and store the transformed image to cache, see `TXImageTransformer`.
 ...
 */
@property (strong, nonatomic, nullable) id<TXImageTransformer> transformer;

/**
 * The cache filter is used to convert an URL into a cache key each time TXWebImageManager need cache key to use image cache.
 *
 * The following example sets a filter in the application delegate that will remove any query-string from the
 * URL before to use it as a cache key:
 *
 * @code
 TXWebImageManager.sharedManager.cacheKeyFilter =[TXWebImageCacheKeyFilter cacheKeyFilterWithBlock:^NSString * _Nullable(NSURL * _Nonnull url) {
    url = [[NSURL alloc] initWithScheme:url.scheme host:url.host path:url.path];
    return [url absoluteString];
 }];
 * @endcode
 */
@property (nonatomic, strong, nullable) id<TXWebImageCacheKeyFilter> cacheKeyFilter;

/**
 * The cache serializer is used to convert the decoded image, the source downloaded data, to the actual data used for storing to the disk cache. If you return nil, means to generate the data from the image instance, see `TXImageCache`.
 ...
 * @code
 TXWebImageManager.sharedManager.cacheSerializer = [TXWebImageCacheSerializer cacheSerializerWithBlock:^NSData * _Nullable(UIImage * _Nonnull image, NSData * _Nullable data, NSURL * _Nullable imageURL) {
 ...
 */
@property (nonatomic, strong, nullable) id<TXWebImageCacheSerializer> cacheSerializer;
...
/**
 ...
 @endcode
 */
@property (nonatomic, strong, nullable) id<TXWebImageOptionsProcessor> optionsProcessor;
...
/**
 ...
 Defaults to nil. Means using `TXImageCache.sharedImageCache`
 */
@property (nonatomic, class, nullable) id<TXImageCache> defaultImageCache;

/**
 The default image loader for manager which is created with no arguments. Such as shared manager or init.
 Defaults to nil. Means using `TXWebImageDownloader.sharedDownloader`
 */
@property (nonatomic, class, nullable) id<TXImageLoader> defaultImageLoader;

/**
 * Returns global shared manager instance.
 */
@property (nonatomic, class, readonly, nonnull) TXWebImageManager *sharedManager;

/**
 * Allows to specify instance of cache and image loader used with image manager.
 * @return new instance of `TXWebImageManager` with specified cache and loader.
 */
- (nonnull instancetype)initWithCache:(nonnull id<TXImageCache>)cache loader:(nonnull id<TXImageLoader>)loader NS_DESIGNATED_INITIALIZER;
