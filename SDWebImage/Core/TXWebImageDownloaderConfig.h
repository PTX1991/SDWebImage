...
typedef NS_ENUM(NSInteger, TXWebImageDownloaderExecutionOrder) {
    /**
     * Default value. All download operations will execute in queue style (first-in-first-out).
     */
    TXWebImageDownloaderFIFOExecutionOrder,
    
    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    TXWebImageDownloaderLIFOExecutionOrder
};
 ...
 */
@interface TXWebImageDownloaderConfig : NSObject <NSCopying>

/**
 Gets the default downloader config used for shared instance or initialization when it does not provide any downloader config. Such as `TXWebImageDownloader.sharedDownloader`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TXWebImageDownloaderConfig *defaultDownloaderConfig;
...
/**
 * Gets/Sets a subclass of `TXWebImageDownloaderOperation` as the default
 * `NSOperation` to be used each time SDWebImage constructs a request
 * operation to download an image.
 * Defaults to nil.
 * @note Passing `NSOperation<TXWebImageDownloaderOperation>` to set as default. Passing `nil` will revert to `TXWebImageDownloaderOperation`.
 */
@property (nonatomic, assign, nullable) Class operationClass;

/**
 * Changes download operations execution order.
 * Defaults to `TXWebImageDownloaderFIFOExecutionOrder`.
 */
@property (nonatomic, assign) TXWebImageDownloaderExecutionOrder executionOrder;
 ...
 * For example, if we config [200, 400) but server response is 503, the download will fail with error code `TXWebImageErrorInvalidDownloadStatusCode`.
 ...
 * For example, if we config ["image/png"] but server response is "application/json", the download will fail with error code `TXWebImageErrorInvalidDownloadContentType`.
