...
typedef void(^TXImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^TXImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `TXWebImageManager` when you specify `SDWebImageRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `TXWebImageErrorCacheNotModified` error. (UIImage)
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable TXImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, SDWebImageOptions options, SDWebImageContext * _Nullable context);
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable TXImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<TXWebImageOperation> _Nonnull operation, SDWebImageOptions options, SDWebImageContext * _Nullable context);
 ...
 */
FOUNDATION_EXPORT id<SDProgressiveImageCoder> _Nullable TXImageLoaderGetProgressiveCoder(id<TXWebImageOperation> _Nonnull operation);
 ...
 */
FOUNDATION_EXPORT void TXImageLoaderSetProgressiveCoder(id<TXWebImageOperation> _Nonnull operation, id<SDProgressiveImageCoder> _Nullable progressiveCoder);
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `TXImageLoaderDecodeImageData` or `TXImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
 ...
 @note It's your responsibility to load the image in the desired global queue(to avoid block main queue). We do not dispatch these method call in a global queue but just from the call queue (For `TXWebImageManager`, it typically call from the main queue).
*/
@protocol TXImageLoader <NSObject>
 ...
 */
- (nullable id<TXWebImageOperation>)requestImageWithURL:(nullable NSURL *)url
                                                options:(SDWebImageOptions)options
                                                context:(nullable SDWebImageContext *)context
                                               progress:(nullable TXImageLoaderProgressBlock)progressBlock
                                              completed:(nullable TXImageLoaderCompletedBlock)completedBlock;
