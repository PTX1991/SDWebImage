...
@interface TXAnimatedImageView () {
    ...
    TXAnimatedImagePlaybackMode _playbackMode;
}

@property (nonatomic, strong, readwrite) TXAnimatedImagePlayer *player;
...
@implementation TXAnimatedImageView
#if SD_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    TXAnimatedImageView *imageView = [[TXAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(TXAnimatedImage)]) {
        if (!self.player) {
            id<TXAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [self progressiveAnimatedCoderForImage:image];
            } else {
                provider = (id<TXAnimatedImage>)image;
            }
            // Create animated player
            self.player = [TXAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<TXAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)setPlaybackMode:(TXAnimatedImagePlaybackMode)playbackMode {
    _playbackMode = playbackMode;
    self.player.playbackMode = playbackMode;
}

- (TXAnimatedImagePlaybackMode)playbackMode {
    if (!_initFinished) {
        return TXAnimatedImagePlaybackModeNormal; // Default mode is normal
    }
...
- (void)checkPlay
{
    // Only handle for TXAnimatedImage, leave UIAnimatedImage or animationImages for super implementation control
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<TXAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<TXAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<TXAnimatedImageCoder, SDProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(TXAnimatedImage)] && image.sd_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<TXAnimatedImageCoder> animatedCoder = [(id<TXAnimatedImage>)image animatedCoder];
        if ([animatedCoder conformsToProtocol:@protocol(SDProgressiveImageCoder)]) {
            return (id<TXAnimatedImageCoder, SDProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
