...
@interface TXImageIOCoderFrame : NSObject
...
@implementation TXImageIOCoderFrame
@end

@implementation TXImageIOAnimatedCoder {
    ...
    NSArray<TXImageIOCoderFrame *> *_frames;
    BOOL _finished;
...
+ (SDImageFormat)imageFormat {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"For `TXImageIOAnimatedCoder` subclass, you must override %@ method", NSStringFromSelector(_cmd)]
                                 userInfo:nil];
}
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize options:(NSDictionary *)options {
    ...
    if (!createFullImage) {
        if (preserveAspectRatio) {
            ...
            CGImageRef scaledImageRef = [TXImageCoderHelper CGImageCreateScaled:imageRef size:thumbnailSize];
    ...
    UIImageOrientation imageOrientation = [TXImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable TXImageCoderOptions *)options {
    ...
    CGFloat scale = 1;
    NSNumber *scaleFactor = options[TXImageCoderDecodeScaleFactor];
    if (scaleFactor != nil) {
        scale = MAX([scaleFactor doubleValue], 1);
    }
    
    CGSize thumbnailSize = CGSizeZero;
    NSValue *thumbnailSizeValue = options[TXImageCoderDecodeThumbnailPixelSize];
    ...
    NSNumber *preserveAspectRatioValue = options[TXImageCoderDecodePreserveAspectRatio];
    ...
    BOOL decodeFirstFrame = [options[TXImageCoderDecodeFirstFrameOnly] boolValue];
    if (decodeFirstFrame || count <= 1) {
        ...
        NSMutableArray<TXImageFrame *> *frames = [NSMutableArray array];
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            TXImageFrame *frame = [TXImageFrame frameWithImage:image duration:duration];
            [frames addObject:frame];
        }
        
        NSUInteger loopCount = [self.class imageLoopCountWithSource:source];
        
        animatedImage = [TXImageCoderHelper animatedImageWithFrames:frames];
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format options:(nullable TXImageCoderOptions *)options {
    ...
    CFStringRef imageUTType = [NSData sd_UTTypeFromImageFormat:format];
    NSArray<TXImageFrame *> *frames = [TXImageCoderHelper framesFromAnimatedImage:image];
    ...
    if (options[TXImageCoderEncodeCompressionQuality]) {
        compressionQuality = [options[TXImageCoderEncodeCompressionQuality] doubleValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationLossyCompressionQuality] = @(compressionQuality);
    CGColorRef backgroundColor = [options[TXImageCoderEncodeBackgroundColor] CGColor];
    if (backgroundColor) {
        properties[(__bridge NSString *)kCGImageDestinationBackgroundColor] = (__bridge id)(backgroundColor);
    }
    CGSize maxPixelSize = CGSizeZero;
    NSValue *maxPixelSizeValue = options[TXImageCoderEncodeMaxPixelSize];
    ...
    NSUInteger maxFileSize = [options[TXImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    if (maxFileSize > 0) {
    ...
    if (options[TXImageCoderEncodeEmbedThumbnail]) {
        embedThumbnail = [options[TXImageCoderEncodeEmbedThumbnail] boolValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationEmbedThumbnail] = @(embedThumbnail);
    
    BOOL encodeFirstFrame = [options[TXImageCoderEncodeFirstFrameOnly] boolValue];
...
#pragma mark - TXAnimatedImageCoder
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<TXImageIOCoderFrame *> *frames = [NSMutableArray array];
    
    for (size_t i = 0; i < frameCount; i++) {
        TXImageIOCoderFrame *frame = [[TXImageIOCoderFrame alloc] init];
        frame.index = i;
