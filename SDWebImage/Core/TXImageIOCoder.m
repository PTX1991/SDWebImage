...
+ (instancetype)sharedCoder {
    static TXImageIOCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[TXImageIOCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable TXImageCoderOptions *)options {
    ...
    UIImage *image = [TXImageIOAnimatedCoder createFrameAtIndex:0 source:source scale:scale preserveAspectRatio:preserveAspectRatio thumbnailSize:thumbnailSize options:nil];
    CFRelease(source);
    if (!image) {
        return nil;
    }
...
- (UIImage *)incrementalDecodedImageWithOptions:(TXImageCoderOptions *)options {
    ...
    if (_width + _height > 0) {
        ...
        image = [TXImageIOAnimatedCoder createFrameAtIndex:0 source:_imageSource scale:scale preserveAspectRatio:_preserveAspectRatio thumbnailSize:_thumbnailSize options:nil];
        if (image) {
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format options:(nullable TXImageCoderOptions *)options {
    ...
    if (format == SDImageFormatUndefined) {
        BOOL hasAlpha = [TXImageCoderHelper CGImageContainsAlpha:imageRef];
        if (hasAlpha) {
    ...
    CGImagePropertyOrientation exifOrientation = [TXImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
#else
